...
- `is_final_preprint`: boolean. **Invariant**: per DOI, at most one row has `true` — the highest-version `v\d+` row, and only when `published_doi` is set. The `version="published"` row, all non-latest `v\d+` rows, and any unpaired preprint row are `false`. Maintained by `ingestion/run_pull.py:update_preprint_published_doi`; enforced by `elastic/scripts/audit_schema_drift.py` (see §2.2.1 changelog 2026-05-27).
...
- 2026-05-27 [Jiayu Zhu] **T8 — `is_final_preprint` semantics tightened + existing data reconciled.** Bug: [ingestion/common/records.py:82](../ingestion/common/records.py#L82) wrote `is_final_preprint = bool(published_doi)` per row, so v1 / v2 / vN of the same DOI were ALL `true` whenever Crossref pairing succeeded. [run_pull.py:88](../ingestion/run_pull.py#L88) `_update_by_query` painless script reinforced the same bug by flipping every version of a paired DOI to `true`. The §4.1 dispatcher's [main.py:539](../apps/dispatcher/main.py#L539) version picker (`sort: [is_final_preprint desc, posted_date desc]`) papered over the bug — `posted_date desc` was the real selector, `is_final_preprint` was noise — so T1 happened to pass and the issue lurked until the dispatch_new_pairs workflow's `filter: is_final_preprint=true` selected duplicate rows in a 2025-05-26 smoke test. Three-part fix: (i) `records.py` always writes `false` at ingestion time; (ii) `run_pull.py` two-step pairing — `_update_by_query` forces every row of a paired DOI to `false`, then a targeted `_update/{id}` flips `true` on the single highest-version row (sorted `posted_date desc, version desc`); (iii) new [elastic/scripts/fix_is_final_preprint.py](../elastic/scripts/fix_is_final_preprint.py) one-shot fixer reconciles the existing ~10k real rows in `preprints` (idempotent, `--dry-run` supported). T7 audit extended with two cross-field semantic invariants: per-DOI uniqueness (terms agg + bucket_selector `_count > 1`) and `version="published"` must be `false` (filtered count). Pure semantic invariants — not schema-drift — but kept in the same script because T7 was already the per-index "is the data consistent with the contract" guard. **Lesson**: T7's field-name diff is necessary but not sufficient; per-field VALUE constraints written in the contract must be enforceable as queries, otherwise dispatcher-side workarounds will silently mask puller bugs (the 5-25 sort-tiebreaker workaround was a smell ignored at the time).
