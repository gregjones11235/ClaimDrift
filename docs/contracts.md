...
> **Status (2026-05-29)**: Full pipeline live. 5 sub-agents + supervisor on Vertex AI Agent Engine; memory loop closed via Elastic MCP; Cloud Run dispatcher + Elastic Scheduled Workflow self-driving (T1 real N>0 + T2 scheduled trigger closed); BFF + Next.js frontend live on real data. See Changelog for the full trail.
...
- The entire Next.js + Tailwind + shadcn/ui frontend, deployed to Cloud Run (6 views)
...
- ✅ **Main flow (§4.1)** — Claim Extractor → Drift Analyzer → Citation Finder → Notifier — is wired via the §9.6.1 inverted topology: an ADK `supervisor_agent` on Agent Engine drives the fan-out, while the Elastic Scheduled Workflow (`dispatch_new_pairs`, every 5 min) detects new `(preprint, published)` pairs and POSTs them to the Cloud Run dispatcher. The dispatcher drives the supervisor, persists every agent's output to ES, and sends the Notifier's emails via Gmail. The top-level `workflows/` directory was intentionally dropped — orchestration lives in the supervisor + dispatcher + Elastic Scheduled Workflow, not a separate Elastic Workflow (see §9.6.1 for why Elastic Workflows can't natively invoke Agent Engine).
...
**Deployment status (2026-05-29)**: 5 of 5 agents + supervisor on Agent Engine (`memory_synthesizer` / `notifier` / `claim_extractor` / `drift_analyzer` / `citation_finder`); 4 of 4 MCP tools live. Main-flow orchestration (5f/5g, §9.6) closed: Cloud Run dispatcher + Elastic Scheduled Workflow self-driving. BFF + Next.js frontend live on real data.
...
- 2026-05-28 [Jiayu Zhu] **Pub/Sub decoupling — `/dispatch` + `/run` split (§9.6.1 round 2).** Workflows' `http` step has a hardcoded ~60s connector timeout on Serverless (`timeout: "600s"` passes schema but still ECONNABORTEDs), so a sync ~200s pipeline can't return in time. Fix: Pub/Sub between workflow and pipeline — `/dispatch` does bearer + idempotency + publish + 202 in ~100ms; `/run` is the push target, verifies Google-signed OIDC (`aud` + `email`), awaits the pipeline (subscription `ack-deadline=600s`). Three GCP gotchas: (1) Pub/Sub managed SA needs `roles/iam.serviceAccountTokenCreator` + `roles/run.invoker`; (2) `--push-auth-token-audience` must be explicit or Pub/Sub normalizes scheme to `http://` and the `aud` check fails; (3) `request.url` inside Cloud Run is unreliable — pin via `PUBSUB_PUSH_AUDIENCE`. Obsoletes the sync-pipeline rationale below.
- 2026-05-28 [Jiayu Zhu] **TODO C — SSE adapter shipped (§6.3).** Shared translator [apps/bff/sse_adapter.py](../apps/bff/sse_adapter.py) owns the envelope shape for both dispatcher (producer) + BFF (consumer); persists via new `agent_events` index (§2.2.8), `drift_event_id` back-filled via `update_by_query`. BFF tails by `drift_event_id`/`dispatch_id` with `Last-Event-ID` resume. Envelope rules are regression-locked by `apps/bff/tests/test_sse_adapter.py`; `SSE_REPLAY_GOLDEN=1` replays the golden for evaluators without GCP creds. Three backlog gotchas: (1) elasticsearch-py 8.x rejects `body=` on `update_by_query` — pass `query=`/`script=` kwargs or it no-ops; (2) `refresh_interval=5s` leaves the last bulk batch buffered when `update_by_query` runs — added an explicit `indices.refresh`; (3) fire-and-forget 202 couldn't scale `/run` (autoscaler counts in-flight HTTP, not `asyncio.create_task`) — dropped for an awaited pipeline, now superseded by the Pub/Sub split above.
- 2026-05-29 [Jiayu Zhu] **Frontend + BFF live on real data; Cloud Run deploy artifacts added (hosted-demo layer).** `frontend/` (Next.js 16 + React 19, 6 views) wired to the BFF via `NEXT_PUBLIC_BFF_URL` (REST `fetch` + browser `EventSource`); no more mock data. BFF (`apps/bff/server.py`) hardened for Cloud Run (honors `$PORT`, binds `0.0.0.0` when containerized). New deploy artifacts keep the whole demo on GCP (no third-party PaaS): `apps/bff/{Dockerfile,cloudbuild.yaml,requirements.txt}` (repo-root build context, same as dispatcher) + `frontend/{Dockerfile,cloudbuild.yaml,.dockerignore}` (`NEXT_PUBLIC_BFF_URL` baked in at build via `--build-arg`). Root README gains a "Deploy the hosted demo" section; `frontend/README.md` rewritten pending → live. Repo hygiene: removed 20 turbopack build artifacts committed under a malformed absolute-path dir (`frontend/Y…/.next/...`) that escaped `.gitignore`.
- 2026-05-31 [Jiayu Zhu] **E3 — pattern_curator correctness + live-retrieval latency isolation verified on real cluster.** Two read-only probes under `agents/pattern_curator/scripts/`: (1) `_e2e_probe.py` plants 2 duplicates + 1 garbage row, asserts hygiene recomputes support_count, ELSER recall pairs the duplicates, and the real Gemini merge is schema-gated (conservative default holds); (2) `_e3_latency_probe.py` — `search_drift_patterns` p95 411ms (curator idle) → 420ms (under dry-run load), +2.3%, well under the 50% bar; confirms the dedicated `claimdrift-elser-batch` endpoint + dry-run isolation shield live reads (design B.2).
- 2026-05-31 [Jiayu Zhu] **Hybrid retrieval RESTORED (reverses the 2026-05-23 single-MATCH simplification).** Single-MATCH had left `_build_rrf_query`'s RRF as ELSER⊕ELSER (lexical leg dead). Fix adds a real lexical leg with zero write-code change: `copy_to` a `text` mirror field off the semantic_text field (both `drift_patterns{,_v2}.json`), point the BM25 sub-retriever at it. Retrofitted the live index in place via `elastic/scripts/retrofit_hybrid_lexical.py --apply` (35/35 docs); verified by `verify_hybrid_fix.py` + 53 tests. **Correction:** first deploy wrongly used `manage_pattern_alias.py rebuild`, parking the read alias on the `drift_patterns_v2` SHADOW index (blue-green is for the rare curator backfill B.1 only); corrected via in-place retrofit + `rollback --apply`. **Steady state: alias `drift_patterns_read` → `drift_patterns`, reads==writes, no shadow.**
- 2026-05-31 [Jiayu Zhu] **E4 — de-hardcoded structured retrieval generalizes off-fixture (both layers PASS).** Layer 1 (`e4_generalization_probe.py`): inject 2 out-of-fixture patterns (economics, neuroscience), query with structured descriptors (no hard-coded hint) — both at hybrid rank 1; hybrid==ELSER-only on this semantically-clean corpus (lexical leg's value is proper-noun/abbreviation queries the 35-pattern corpus rarely exercises). Layer 2 (`e4_e2e_{setup,check}.py`): `drift_analyzer` end-to-end on an economics minimum-wage case built an economics query, used the economics pattern, did NOT misuse the clinical outcome-switch memory; 7/7 pass. Confirms the `agent.py:74-80` hint removal (design D2) is real.
- 2026-05-31 [Jiayu Zhu] **E2 (accumulation curve) — data-source decision: synthetic *controlled* base rate over real cases, NOT real accumulation.** Two constraints forced it: (1) reproducibility — the loop is append-only, so a support=120 pattern can't be returned to support=50 for a re-film; only independently-constructed per-bucket state is reproducible; (2) real accumulation ≈ 0 — a live audit found 391 drift_events / 35 patterns / **zero `outcome_switch`** across 10,671 preprints. So E2 is a controlled experiment: `support_count` set as the independent variable, `calibrated_materiality` the measured response, sourced from real COMPARE outcome-switch cases as `source_event_ids`. Curve buckets + real-case harvest closed in the next entry.
- 2026-05-31 [Jiayu Zhu] **E2 (accumulation curve) — CLOSED; blind run is the trustworthy result.** Harvested 51 real DOI-bearing outcome-switch cases from COMPARE (`agents/evals/fixtures/compare_outcome_switch_cases.json`); traceable set caps tiers at **[5,20,35,50] all-real** (not 120). Harness `agents/scripts/e2_accumulation_curve.py` injects N cases per tier as one pattern's `source_event_ids`, torn down between tiers so it can't corrupt the §3.6 base rate; dependent variable is a real ambiguous case (tasimelteon SET, NCT01163032). **Methodology finding:** the first pass leaked the tier number into the prompt → a prettier-but-anchored curve; the reported run is **blind** (prompt byte-identical across tiers; agent reads `support_count` from ES). Blind result **0.75 / 0.75 / 0.80 / 0.82** (no-memory 0.50) proves (1) the memory loop works (0.50→0.75) and (2) the agent genuinely reads the ES base rate (`evidence[].support_count` matches the injected value per tier). Summary: `agents/evals/results/memory-loop-e2-2026-05-31/demo_summary.md`. **Lesson: for any "curve vs X" experiment, first check whether X leaked from the prompt.**
